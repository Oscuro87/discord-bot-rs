...
pub async fn send(ctx: &Context, target_channel: ChannelId, reply: MessageBuilder) -> () {
    send_or_forward_err(ctx, target_channel, reply).await;
}

pub async fn send_or_console_err(ctx: &Context, target_channel: ChannelId, reply: MessageBuilder) -> () {
    if let Some(err) = send_or_forward_err(ctx, target_channel, reply).await {
        println!("Error: {}", err);
    }
}

pub async fn send_or_discord_err(ctx: &Context, target_channel: ChannelId, error_target_channel: ChannelId, reply: MessageBuilder) -> () {
    if let Some(err) = send_or_forward_err(ctx, target_channel, reply).await {
